...
class MeterProviderTest {
    ...
    @Mock
    private MeterBuilder meterBuilderMock;

    @Captor
    private ArgumentCaptor<String> meterNameCaptor;
    ...
    void ensureThatConfigureMetersHaveUniqueIdentity() {
        try (MockedStatic<GlobalOpenTelemetry> otelMock = mockStatic(GlobalOpenTelemetry.class)) {
            otelMock.when(() -> GlobalOpenTelemetry.meterBuilder(anyString())).thenReturn(meterBuilderMock);
            ...
            otelMock.verify(() -> GlobalOpenTelemetry.meterBuilder(meterNameCaptor.capture()), atLeast(1));
