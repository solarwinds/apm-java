...
class ResourceAttributesToSystemPropertiesTest {
  @InjectMocks private ResourceAttributesToSystemProperties tested;

  @Mock private AutoConfiguredOpenTelemetrySdk sdkMock;

  @Test
  void verifyThatServiceNameIsInjectedIntoSystemProperty() {
    try (MockedStatic<AutoConfiguredResourceCustomizer> resourceCustomizerMock =
        mockStatic(AutoConfiguredResourceCustomizer.class)) {
      resourceCustomizerMock
          .when(AutoConfiguredResourceCustomizer::getResource)
